...
public class ItemCandle extends ArsMagicaItem{
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (!stack.hasTagCompound() || !stack.getTagCompound().hasKey("search_block")){
			...
			if (player.isSneaking() && block != null && block.getBlockHardness(world, pos) > 0f && world.getTileEntity(pos) == null){
				if (!world.isRemote){
					setSearchBlock(world.getBlockState(pos).getBlock(), world.getBlockMetadata(x, y, z), stack);
		...
		if (!world.isRemote){
			...
			Block block = world.getBlockState(pos).getBlock();
			if (block == null || block.isReplaceable(world, pos)){
				int newMeta = (int)Math.ceil(stack.getItemDamage() / 1200);
				world.setBlockState(pos, BlocksCommonProxy.candle.getDefaultState(), newMeta);
		...
		for (int i = -radius; i <= radius; ++i){
			for (int j = -1; j <= 1; ++j){
				for (int k = -radius; k <= radius; ++k){
					Block f_block = world.getBlockState(new BlockPos(cx + i, cy + j, cz + k)).getBlock();
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int indexInInventory, boolean isCurrentlyHeld){
		if (isCurrentlyHeld && entity instanceof EntityPlayer){
			...
			if (!world.isRemote && AMCore.config.candlesAreRovingLights() &&
				...
				world.setBlockState(new BlockPos((int)Math.round(entity.posX), (int)Math.round(entity.posY), (int)Math.round(entity.posZ)), BlocksCommonProxy.invisibleUtility.getDefaultState(), 2);
			}
		}
	}
