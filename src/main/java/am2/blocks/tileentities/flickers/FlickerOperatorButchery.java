...
public class FlickerOperatorButchery implements IFlickerFunctionality{
	...
	public boolean DoOperation(World worldObj, IFlickerController habitat, boolean powered){
		...
		int radius = 6;
		List<EntityAnimal> creatures = worldObj.getEntitiesWithinAABB(EntityAnimal.class, new AxisAlignedBB(((TileEntity)habitat).getPos().getX() - radius, ((TileEntity)habitat).getPos().getX() - radius, ((TileEntity)habitat).getPos().getZ() - radius, ((TileEntity)habitat).getPos().getX() + radius + 1, ((TileEntity)habitat).getPos().getY() + radius + 1, ((TileEntity)habitat).getPos().getZ() + radius + 1));
		for (EntityAnimal creature : creatures){
			...
			if (count > 2){
				if (worldObj.isRemote){
					AMParticle particle = (AMParticle)AMCore.proxy.particleManager.spawn(worldObj, "ghost", ((TileEntity)habitat).getPos().getX() + 0.5, ((TileEntity)habitat).getPos().getX() + 0.7, ((TileEntity)habitat).getPos().getZ() + 0.5);
					if (particle != null){
